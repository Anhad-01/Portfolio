...
/**
 * MagicBento - Accepts external card data via `cards` prop.
 * Each card object: { title, description, label, image, href, color }
 * Large cards are index 2 (col 3-4, row 1-2) and index 3 (col 1-2, row 2-3).
 */
const MagicBento = ({
  cards = [],
  textAutoHide = true,
        ...
        {`
          ...
          @media (min-width: 1024px) {
            .card-responsive {
              ...
              grid-template-rows: 230px 40px 230px;
            }
      ...
      <BentoCardGrid gridRef={gridRef}>
        <div className="card-responsive grid gap-2">
          {cards.map((card, index) => {
            ...
            const baseClassName = `card flex flex-col justify-between relative min-h-[180px] w-full max-w-full p-5 rounded-[20px] border border-solid font-light overflow-hidden transition-colors duration-300 ease-in-out hover:-translate-y-0.5 hover:shadow-[0_8px_25px_rgba(0,0,0,0.15)] ${enableBorderGlow ? 'card--border-glow' : ''
              }`;
              ...
              <>
                ...
                {card.image && (
                  <div className="absolute inset-0 z-0">
                    ...
                    />
                    <div className="absolute inset-0 bg-gradient-to-t from-[#120F17] via-[#120F17]/40 to-transparent" />
                  </div>
                )}
                <div className="card__header flex justify-between gap-3 relative z-10 text-white">
                  <span className="card__label text-base font-bold">{card.label || card.title}</span>
                </div>
                <div className="card__content flex flex-col relative z-10 text-white">
                  {card.label && (
                    <h3 className={`card__title font-semibold text-lg m-0 mb-1 ${textAutoHide && !isLarge ? 'text-clamp-1' : ''}`}>
                      {card.title}
                    </h3>
                  )}
                  <p className={`card__description text-xs leading-5 opacity-70 ${textAutoHide && !isLarge ? 'text-clamp-2' : ''}`}>
                    {card.description}
                  </p>
